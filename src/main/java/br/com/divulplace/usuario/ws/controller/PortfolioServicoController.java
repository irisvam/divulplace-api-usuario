...
public class PortfolioServicoController extends CommonController {
	...
	@Autowired
	private PortfolioServicoService repServico;
	...
	public ResponseEntity<List<UserPortfolioServico>> recuperarPortfolioServico(@PathVariable(value = "id") final Long idAfiliado) {
		...
		return new ResponseEntity<List<UserPortfolioServico>>(this.repServico.encontrarListaPortfolioServicos(idAfiliado), HttpStatus.OK);
	}
	...
	public ResponseEntity<RetornoCadastro> cadastrarPortfolioServico(@PathVariable(value = "id") final Long idUsuario,
		...
		final RetornoCadastro retorno = this.repServico.cadastrarPortfolioServico(afiliado, userServico);
	...
	@PutMapping("/{id}")
	public ResponseEntity<?> atualizarPortfolioServico(@PathVariable(value = "id") final Long idUsuario,
			@Valid @RequestBody final UserPortfolioServico userServico) {
		...
		if (!this.repServico.atualizarPortfolioServico(userServico)) {
